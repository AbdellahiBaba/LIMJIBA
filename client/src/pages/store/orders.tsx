...
import { Search, Package, Clock, CheckCircle2, Truck, PackageCheck, XCircle, Mail } from "lucide-react";
...
export default function StoreOrders() {
  ...
  const buildLookupUrl = () => {
    if (isAuthenticated) {
      return `/api/store/orders/lookup`;
    }
    const params = new URLSearchParams();
    if (orderNumberQuery.trim()) params.set("orderNumber", orderNumberQuery.trim());
    if (emailQuery.trim()) params.set("email", emailQuery.trim());
    return `/api/store/orders/lookup?${params.toString()}`;
  };

  const canSearch = isAuthenticated || (!!orderNumberQuery.trim() && !!emailQuery.trim());

  const { data: orders, isLoading, refetch } = useQuery<StoreOrder[]>({
    queryKey: ["/api/store/orders/lookup", orderNumberQuery, emailQuery, isAuthenticated],
    queryFn: async () => {
      if (!canSearch) return [];
        ...
        <div className="space-y-3 mb-8">
          <div className="relative">
            ...
            <Input
              ...
              onChange={e => { setOrderNumberQuery(e.target.value); setSearched(false); }}
          ...
          <div className="relative">
            ...
            <Input
              placeholder={t("orders.emailPlaceholder") || "Email address"}
              type="email"
              value={emailQuery}
              onChange={e => { setEmailQuery(e.target.value); setSearched(false); }}
              onKeyDown={e => e.key === "Enter" && handleSearch()}
              className="pl-10 rounded-full"
              data-testid="input-order-email"
            />
          </div>
          <Button onClick={handleSearch} disabled={!canSearch} className="rounded-full px-6" style={{ backgroundColor: accentColor, color: primaryColor }} data-testid="button-search-order">
            {t("orders.search")}
          </Button>
        </div>
      )}
                  ...
                  {orderItems.length > 0 && (
                    <div className="space-y-2 mb-4">
                      {orderItems.map((item: any, idx: number) => (
                        <div key={idx} className="flex justify-between gap-2 flex-wrap text-sm">
