...
- **Customer Loyalty Program:** `loyaltyPoints` field on `store_customers` table. Awards 1 point per 10 MRU spent when orders are delivered. Points badge in store header, display in profile page, earn preview in checkout.
...
- **SVG Logo Animation:** Al-Jazeera-style SVG line-drawing animation on homepage hero. Renders world map dots, angular shape, camel with rider, airplane, Arabic text "لمجيبة", and "IMPORTING" text with sequential stroke-draw reveal animations. Self-contained SVG with inline CSS keyframes, stable instance IDs (useRef), and `prefers-reduced-motion` media query support. Component: `client/src/components/svg-logo-animation.tsx`. Old canvas-based particle animation still exists at `client/src/components/logo-animation.tsx` but is no longer imported.
