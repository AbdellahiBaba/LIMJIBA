...
- **Logo Particle Assembly Animation:** Pixel-sampled Canvas animation on homepage hero. Logo image is loaded offscreen, dark pixels sampled to create ~180 particle target positions that form the logo silhouette. Golden particles, lines, and geometric shapes (triangles/squares/pentagons/hexagons) scatter randomly then converge to assemble the logo shape, hold with subtle glow, then disperse — continuous 8-second loop. No `<img>` tag in output (reduced-motion fallback shows static image). Component: `client/src/components/logo-animation.tsx`.
