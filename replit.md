...
- **Pages:** home (premium hero + trust badges + categories + featured + deals of the day + recently viewed + recommendations), products (premium cards + category pills + comparison tool), product detail (premium layout + secure/delivery badges + recently viewed tracking), cart (premium summary + gold accents), checkout (with payment step + loyalty points earn preview), orders (with visual timeline), about (brand story + values), contact (premium cards + WhatsApp), terms, login, signup, profile (with loyalty points display), compare (side-by-side product comparison).
