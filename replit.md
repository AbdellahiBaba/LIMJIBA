...
- **Logo Ambient Animation:** Continuous looping Canvas animation on homepage hero. Logo image always visible at center with `animate-float`. Golden particles, glowing lines, and geometric shapes (triangles/diamonds/hexagons) orbit around the logo in a 6-second cycle: scatter → converge toward logo edges → orbit/shimmer → fade → repeat. ~100 particles, ~12 geo shapes, ~25 connecting lines. Respects `prefers-reduced-motion`. Component: `client/src/components/logo-animation.tsx`.
