...
async function runMigrations(): Promise<void> {
  ...
  try {
    ...
    if (!existingCols.includes('customer_email')) {
      await client.query(`ALTER TABLE sales ADD COLUMN customer_email text`);
      console.log('[DB] Added customer_email column to sales');
    }
